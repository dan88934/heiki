...
class TestInput(unittest.TestCase): 
    ...
    def tearDown(self): 
        for filename in os.listdir("./Heiki_V1/uploaded_files"): #This removes the files which the tests can upload
            r_file = "./Heiki_V1/uploaded_files/" + filename #Removes all the files in a directory
            os.remove(r_file)
    ...
    def test_title_input(self): #Checks to make sure that the user cannot submit form without title value
        ...
        start_count = 0
        for path in pathlib.Path("./Heiki_V1/uploaded_files").iterdir():
            if path.is_file():
                start_count += 1
        ...
        title.click()
        title.send_keys('')
        file_type = Select(self.driver.find_element_by_id('caption'))
        file_type.select_by_value('image')
        file_input = self.driver.find_element_by_id("file")
        absolute_file_path = os.path.abspath("./tests/img_04.png")
    ...
    def test_all_correct_input(self): #Test that file arrives in output_files when correct input is given
        ...
        start_count = 0
        for path in pathlib.Path("./Heiki_V1/output_files").iterdir(): 
            if path.is_file():
                start_count += 1
        ...
        for filename in os.listdir("./Heiki_V1/output_files/"): #This removes the files which the tests can upload
            r_file = "./Heiki_V1/output_files/" + filename #Removes all the files in a directory
            os.remove(r_file)
